...
function check(label: string, ring: Ring, ov: Partial<Params>) {
  ...
  let rects = 0, bad = 0, outside = 0, blocks = 0;
  for (const q of b.parcels) {
    ...
    if (q.envelope) {
      ...
      if (out > 0.05) { outside++; console.log(`  #${q.no} zarf dışı taşma: ${out.toFixed(3)} m²`); }
    }
  }
  console.log(`${label}: parsel=${b.parcels.length} geçerli=${b.parcels.filter(x=>x.valid).length} blok=${blocks} dikAçılıDikdörtgen=${rects} bozuk=${bad} zarfDışıTaşan=${outside}`);
}
...
try {
  ...
  for (const [i, r] of polygonsOfLayer(d, layer).entries()) check(`350ADA#${i + 1} 290-330`, r, { minArea: 290, maxArea: 330 });
} catch (e) { console.log("350ADA yok:", (e as Error).message); }
